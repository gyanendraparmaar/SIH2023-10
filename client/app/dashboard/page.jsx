...
export default function Page() {
    ...
    return (
        <div className="min-h-full w-full bg-slate-100 grid grid-cols-5 gap-0">
            <div id="departments" className="bg-slate-700 h-full col-span-1 flex flex-col gap-2 items-center justify-center">
                {
                    Object.keys(departments).map(i => <div data-num={i} key={i} onClick={changeDept} className={`cursor-pointer ${departments[parseInt(i)].active ? "rounded-r-none" : "rounded-full"} ${departments[parseInt(i)].active ? "hover:bg-slate-100" : "hover:bg-slate-600"} p-5 w-full text-center rounded-full ${departments[parseInt(i)].active ? "bg-slate-100" : "bg-transparent"} ${departments[parseInt(i)].active ? "text-slate-500" : "text-slate-300"} font-bold text-2xl`}>{departments[parseInt(i)].dept}</div>)
                }
            </div>
            <div className="h-full col-span-4 flex flex-col items-center">
                ...
                <div className="h-full w-full p-10 flex flex-wrap gap-5">
                    {
                        Object.keys(newsCards).map(i => <div key={i} className="relative w-96 h-96 p-5 rounded-md flex flex-col gap-1" style={{ backgroundColor: `${interpolateLab("#ff7f69", "#a5ff69")(newsCards[i].sentiment)}` }}>
                            <div className="text-center text-slate-700 text-base font-bold leading-6 max-h-12 overflow-hidden text-ellipsis line-clamp-2 mb-1">{ newsCards[i].headline }</div>
                            <hr className="w-full border-1 border-slate-100"></hr>
                            <div className="relative h-48 text-slate-700 overflow-hidden text-ellipsis tracking-wide m-3" style={{
                                overflow: "hidden",
                                display: "-webkit-box",
                            ...
                            <div className="absolute bottom-0 w-full h-16 -translate-x-5 backdrop-blur-sm cursor-pointer rounded-md flex p-3 items-center" style={{ backgroundColor: "rgba(255, 255, 255, 0.2)" }}>
                                <p className="text-slate-700"><span className="text-xl p-2">🙂</span> { newsCards[i].sentiment * 100 }%</p>
                            </div>
                        </div>)
                    }
                </div>
